...
import { EthersLiquityWithStore } from "./lib-ethers/EthersLiquity.js";
...
export const tryToLiquidate = async (
  ...
  const maxPriorityFeePerGas = BigNumber.from(config.maxPriorityFeePerGas);

  const maxFeePerGas = baseFeePerGas.mul(2).add(5e9);
  ...
  try {
    ...
    const { collateralGasCompensation, bfeeGasCompensation, liquidatedAddresses, minerCut } =
      receipt.details;
