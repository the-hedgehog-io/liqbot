...
import stabilityPoolAbi from "./abi/StabilityPool.json" assert { type: 'json' };
import defaultPoolAbi from "./abi/DefaultPool.json" assert { type: 'json' };
import activePoolAbi from "./abi/ActivePool.json" assert { type: 'json' };
import troveManagerAbi from "./abi/TroveManager.json" assert { type: 'json' };
import multiTroveGetterAbi from "./abi/MultiTroveGetter.json" assert { type: 'json' };
import priceFeedAbi from "./abi/PriceFeed.json" assert { type: 'json' };
import sortedTrovesAbi from "./abi/SortedTroves.json" assert { type: 'json' };

import {
  TroveManager,
  MultiTroveGetter,
  PriceFeed,
  SortedTroves, DefaultPool, ActivePool, StabilityPool
} from "./ether-types.js";
...
export type _TypedLiquityContract<T = _LiquityContract, U = unknown> = TypedContract<_LiquityContract, T, U>;
...
export const _priceFeedIsTestnet = (
  priceFeed: PriceFeed
): priceFeed is PriceFeed => "setPrice" in priceFeed;
...
const getAbi = (): LiquityContractAbis => ({
...
export interface _LiquityDeploymentJSON {
  ...
  readonly totalStabilityPoolHOGReward: string;
  readonly liquidityMiningHOGRewardRate: string;
...
export const _connectToContracts = (
  ...
  ) as unknown as _LiquityContracts;
};
